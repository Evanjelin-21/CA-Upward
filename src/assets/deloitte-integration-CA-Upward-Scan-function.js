...
function startExecForScan(vaultId, token) {
  ...
  if (!hasLoadingFailed) {
    ...
    ith.createInstance(document.getElementById("imageTrustDiv"), ithExamplesConfig)
      .then(transactionalClientCreatedForScan)
...
function validateAndLoadDocumentForScan(batch) {
  ...
  // hideToolbarMenuForScan(); //Hide Menus
  disableClickActionForScan(); //Disable Click Action
  // disableActionForScan(); //Disable Action
  ...
  //   //   dmsUser: user,
  //   //   endpoint: docuEdgeEndPoint,
  //   //   token: getUserTokenForScan()
  //   //   //token: tempCurrentUser.token
  //   // }));
  //   // getDocURLForScan(startExecToken, docId).then(function (data) {
...
function setDocumentTitlesForScan() {
...
//Set default names
function updateDefaultNodeNamesForScan() {
...
function uploadAsVersionHandlerForScan() {
  if (!isInvalidDocument) {
    SplitConfirm.show('Do you wish to upload as version to the document?', changeMetadaForScan);
  } else {
    SplitConfirm.show('Invalid Document. Do you wish to discard the changes?', reloadPageForScan);
  }
}

//Handler for saving the document and not actually passing review
function saveHandlerForScan() {
  if (!isInvalidDocument) {
    console.log(SplitConfirm);
    uploadAsNewForScan()
    //SplitConfirm.show('Do you wish to save your work and come back to it later?', removeRedactedDataFromJsonForScan);
  } else {
    SplitConfirm.show('Invalid Document. Do you wish to discard the changes?', reloadPageForScan);
  }
}
...
//Handler for saving the document and not actually passing review
function cancelHandlerForScan() {
  if (!isInvalidDocument) {
    SplitConfirm.show('Are you sure you wish to cancel redaction? Unsaved changes will be lost!', closeWindowForScan);
  } else {
    SplitConfirm.show('Invalid Document. Do you wish to discard the changes?', reloadPageForScan);
  }
}
...
function uploadAsNewHandlerForScan() {
  if (!isInvalidDocument) {
    SplitConfirm.show('Do you wish to upload as new document?', uploadAsNewForScan);
  } else {
    SplitConfirm.show('Invalid Document. Do you wish to discard the changes?', reloadPageForScan);
  }
}

function reloadPageForScan() {
...
function setErrorStylingForScan() {
  //uploadNewButton.style.display = "none"
  uploadVersionButton.style.display = "none"
  ithInstanceDiv.style.display = "none"
  ithErrorDiv.style.display = "block"
...
function changeMetadaForScan() {
  documentDetails['metadata']['properties']['Passed Review'] = true;
  uploadAsVersionForScan()
}


function uploadAsVersionForScan() {
...
async function uploadAsNewForScan() {
  ...
  console.log(documentsCount, hasMultipleDocuments, allDocumentDetails,)
  var title = 'test'
  ithInstance.currentBatch.documents.forEach((element, index) => {
  ...
  for (let index = 0; index < ithInstance.currentBatch.documents.length; index++) {
    ...
    progressModal.closeFn();
    progressModal = ithInstance.indeterminateProgress("Sending document...", "Sending");
    responsePromise.then(async function(blob) {
    var annotatedFile = new File([blob], docUuids[index], {
...
responsePromise.catch(function(error) {
    progressModal.closeFn();
    ithInstance.alert("Failed to send the batch to the webserver with error: " + error);
});
...
function hideToolbarMenuForScan() {
...
function disableActionForScan() {
  ithInstance.registerActionAllowCheck(ith_ActionType.IMPORT_DRAGGED_FILES, disallowForScan);
  ithInstance.registerActionAllowCheck(ith_ActionType.ROTATE_PAGE, allowForScan);
}

function allowForScan(targetNode, selectedNodes) {
  return true;
}

function disallowForScan(targetNode, selectedNodes) {
  return false;
}

function closeWindowForScan() {
  window.close()
  ithInstance.deleteBatch();
  //iframe.postMessage("close window", "*")
}

function disableClickActionForScan() {
  ithInstance.setActionVisibility(ith_ActionType.APPEND_NEW_DOCUMENT, false);
  ...
  ithInstance.setActionVisibility(ith_ActionType.NEW_BATCH_ACTION, true);
  ...
  ithInstance.setActionVisibility(ith_ActionType.OPEN_BATCH_ACTION, true);
  ithInstance.setActionVisibility(ith_ActionType.ROTATE_PAGE, true);
  ithInstance.setActionVisibility(ith_ActionType.SCAN_ALL_AND_APPEND, true);
...
function transactionalClientCreatedForScan(dispatch) {
 //document.getElementsByClassName('itw-panel itw-bep-panel itw-resizable')[0].style.display = 'none'
  ...
  ithInstance.JobEvents.batchStructureChanged = onBatchStructureChangedForScan;  //DO
...
function initForScan() {
  ...
  function newBatchCreatedForScan(batch) {
    ...
    validateAndLoadDocumentForScan(batch);
    ithInstance.scanAllAndAppend()
...
//       <button class="button" id="upload-version" onclick="uploadAsVersionHandlerForScan()" style="display: block; background-color: #104f7d; color: white">Complete</button>
//       <button class="button" id="save-document" onclick="saveHandlerForScan()" style="background-color: #104f7d; color: white">Save</button>
//       <button class="button" id="cancel-redact" onclick="cancelHandlerForScan()" style="background-color: #104f7d; color: white">Back</button>
...
function setupListenersForScan() {
  /** @type {ISearchListener} */
  var searchListener = {};
  searchListener.searchModeDeactivated = onSearchModeDeactivatedForScan;
  searchListener.searchModeActivated = onSearchModeActivatedForScan;

  /** @type {IViewerListener} */
  var viewerListener = {};
  viewerListener.pageChanged = onPageChangedForScan;
  viewerListener.pageUpdated = onPageUpdatedForScan;
...
function installSearchTabsForScan() {
  ...
  elCustomFilterboxInput.addEventListener("input", onFilterInputForScan, true);
  ...
  elPanelResultsHeaderCheckBox.addEventListener("click", onHeaderCheckBoxClickedForScan);
  ...
  elCustomRedactBtn.addEventListener("click", redactBtnClickedForScan);
 ...
 * @param s {string}
 */
function doesFilterMatchesForScan(s) {
 ...
 * @param ev {InputEvent}
 */
function onFilterInputForScan(ev) {
 ...
 * @param ev {MouseEvent}
 */
function onHeaderCheckBoxClickedForScan(ev) {
...
function onSearchModeActivatedForScan() {
  console.log("[Custom searchListener]: search mode activated");
  searchModeActive = true;
  if (!customSearchTabInstalled) {
    installSearchTabsForScan();
  }
  customSearchTabInstalled = true;
  clearHighlightsForScan();
  var p = getSelectedPageForScan();
  if (!p) {
    return;
  }
  console.log("[Custom searchListener]: redrawing highlights");
  drawHighlightsForScan(p.levelIndex + 1);
  if (typeof activeRecordId === "number" && activeRecordId > -1) {
    setActiveForScan(activeRecordId);
  }
}

function onSearchModeDeactivatedForScan() {
...
function onPageChangedForScan(viewer, currentPage, previousPage) {
  if (!searchModeActive) {
    return;
  }
  ...
  clearHighlightsForScan();
  drawHighlightsForScan(currentPage.levelIndex + 1);
  if (typeof activeRecordId === "number" && activeRecordId > -1) {
    setActiveForScan(activeRecordId);
  }
}

function onPageUpdatedForScan(viewer, page, action) {
  if (!searchModeActive) {
    return;
  }
  ...
  clearHighlightsForScan();
  drawHighlightsForScan(page.levelIndex + 1);
  if (typeof activeRecordId === "number" && activeRecordId > -1) {
    setActiveForScan(activeRecordId);
  }
}
...
/** @type {PIIRecord[]}*/
var piiRecords = [];
function processPIIDataForScan() {
...
function PIIRecord(pageNum, text, boundingBox, confidenceScore, id, recType) {
  ...
  function onCheckBoxClicked(ev) {
    ...
    if (this.highlight) {
      ...
      applyStateToHighlightForScan(this.highlight, this.highlight.normalState);
    }
    updateRedactBtnForScan();
  }
 ...
 */
function getSelectedPageForScan() {
 ...
 * @param {MouseEvent} ev
 */
function redactBtnClickedForScan(ev) {
  ...
  piiRecords.forEach(
    ...
    function (record, idx) {
      ...
      applyStateToHighlightForScan(record.highlight, record.highlight.normalState);
    });
 ...
 * @return {boolean}
 */
function setActiveForScan(recordId) {
  ...
  if (typeof activeRecordId !== "undefined" && activeRecordId != recordId) {
    ...
    var prevActiveRecord = getRecordByIdForScan(activeRecordId);
    if (prevActiveRecord.highlight) {
      applyStateToHighlightForScan(prevActiveRecord.highlight, prevActiveRecord.highlight.normalState);
      prevActiveRecord.el.classList.remove("itw-selected");
    }
  }

  var record = getRecordByIdForScan(recordId);
  if (!record) {
  ...
  applyStateToHighlightForScan(record.highlight, record.highlight.activeState);
}
 ...
 * @return {PIIRecord}
 */
function getRecordByIdForScan(rId) {
  /** @var {PIIRecord} */
  var found = undefined;
...
function updateRedactBtnForScan() {
  var checked = false;
  piiRecords.forEach(function (record, idx) {
 ...
 * @param state
 */
function applyStateToHighlightForScan(hl, state) {
 ...
 * @param pageNum
 */
function drawHighlightsForScan(pageNum) {
  piiRecords.forEach(function (record, idx) {
    var bbox = record.boundingBox;
    if (record.pageNum == pageNum) {
      ...
      removeHighlightForScan(record.highlight);
      record.highlight = undefined;
    }
  });
}

function drawHighlightsUsingBboxesForScan(pageNum, bbox, confidence) {
...
function removeHighlightForScan(hl) {
  if (hl) {
    ithInstance.removeHighlight(hl);
  }
}

function clearAllHighlightsForScan() {
  ithInstance.clearHighlights();
}

function clearHighlightsForScan() {
  piiRecords.forEach(function (record) {
    record.highlight = undefined;
  });
  ithInstance.clearHighlights();
 ...
 */
function doHttpRequestForScan(method, url, xhrCallback, data) {
  data = data || null;
...
function removeRedactedDataFromJsonForScan() {
  redactedRecords = [];
  piiRecords.forEach(
    /**
     * @param {PIIRecord} record
...
async function getDocURLForScan(token, vaultdocId) {
...
async function sendDocForUploadToVault(token, file) {
  ...
  bodyData.append("title", "testFile");
  bodyData.append("category_id", "1");
  bodyData.append("meta_data", JSON.stringify({}));
  // window.postMessage({
  //   "file": file,
  //   "title": "testFile",
  //   "category_id": "1",
  ...
  // }, '*')
  return new Promise(async function (resolve, reject) {
    var url = 'https://upward-docuedgevault.deloitte.com/documents/upload/direct'//74e144b3-557f-44ce-8081-ac935c9be534';
    const response = await fetch(url, { method: 'POST', body: bodyData, headers: header })
    const data = await response.json()
    console.log("Response", data)
    if (response.status == 200) {
      resolve(data)
